...
void Move_Begin (edict_t *ent)
{
	...
	ent->nextthink = level.time + (frames * FRAMETIME);	// when Move_Final gets called to stop translation
...
//	return;	// RSP 103100 - not necessary
}
...
it is triggered, when it will lower and become a normal plat.
...
The "sounds" setting won't matter. There is only one sound.
*/
...
void rotating_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	else
	{
		...
		if (self->spawnflags & SPAWNFLAG_FROT_TOUCH_PAIN)
...
{
	ent->solid = SOLID_BSP;
	if (ent->spawnflags & SPAWNFLAG_FROT_STOP)
	...
	if (ent->spawnflags & SPAWNFLAG_FROT_X_AXIS)
		ent->movedir[2] = 1.0;
	else if (ent->spawnflags & SPAWNFLAG_FROT_Y_AXIS)
		ent->movedir[0] = 1.0;
	else // Z_AXIS
		ent->movedir[1] = 1.0;

	// check for reverse rotation
	if (ent->spawnflags & SPAWNFLAG_FROT_REVERSE)
...
//	ent->moveinfo.sound_middle = "doors/hydro1.wav";
	...
	if (ent->spawnflags & SPAWNFLAG_FROT_START_ON)
		ent->use (ent, NULL, NULL);

	if (ent->spawnflags & SPAWNFLAG_FROT_ANIMATED)
		ent->s.effects |= EF_ANIM_ALL;
	if (ent->spawnflags & SPAWNFLAG_FROT_ANIMATED_FAST)
...
void SP_func_button (edict_t *ent)
{
	vec3_t	abs_movedir;
	float	dist;
	char	button_sound_str[24];	// RSP 032499 - variable button sound

	G_SetMovedir (ent->s.angles, ent->movedir);
	ent->movetype = MOVETYPE_STOP;
	ent->solid = SOLID_BSP;
//ent->s.renderfx |= RF_TRANSLUCENT;	// RSP DEBUG
	gi.setmodel (ent, ent->model);

	// RSP 032499 - We now have 5 button types, 1 -> 5, determined by the setting
	// of the "sounds" key. 
	...
	case 4:
	case 5:
		sprintf(button_sound_str,"switches/butn%d.wav",ent->sounds);
		ent->moveinfo.sound_start = gi.soundindex (button_sound_str);
		break;
	default:
		ent->moveinfo.sound_start = gi.soundindex ("switches/butn1.wav");
		break;
	}
...
void Touch_DoorTrigger (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// WPO 100399 Doors need to open for dreadlock
	if (!(other->svflags & SVF_MONSTER) && (!other->client) && (other->identity != ID_DLOCK))
		return;
...
// RSP 092899 - We now have 6 door types, 1->6, determined by the setting
// of the "sounds" key. 

void SetDoorSounds(edict_t *ent)
{
	char s[24];
	...
	case 3:
	case 4:
	case 5:	// RSP 092899 - new door sound from Biz
	case 6:	// RSP 092899 - new door sound from Biz
		sprintf(s,"doors/dr%d_strt.wav",ent->sounds);
		ent->moveinfo.sound_start  = gi.soundindex(s);
		sprintf(s,"doors/dr%d_mid.wav",ent->sounds);
		ent->moveinfo.sound_middle = gi.soundindex(s);
		sprintf(s,"doors/dr%d_end.wav",ent->sounds);
		ent->moveinfo.sound_end    = gi.soundindex(s);
		break;
	default:
...
// WPO 042800 removed references to all these sounds
#if 0
	switch (self->sounds)
	{
		default:
...
#define TRAIN_ANIMATED		8	// RSP 080499 - add animation
#define TRAIN_ANIMATED_FAST	16	// RSP 080499 - add animation


/*QUAKED func_train (0 .5 .8) ? START_ON TOGGLE BLOCK_STOPS ANIMATED ANIMATED_FAST
...
animated: allow animated textures to work
animated_fast: all animated textures to work real fast
*/
...
// RSP 062999 - train_wait() is called when a func_train hits its destination path_corner
// It doesn't actually _hit_ the path_corner. train_wait() gets called once the func_train
// has traveled the distance needed to get to the next path_corner. At that point,
// self->target_ent is the path_corner the func_train is currently sitting at.
...
again:
	...
	if (ent->spawnflags & SPAWNFLAG_PC_TELEPORT)
	{
		if (!first)
		{
...
void SP_func_train (edict_t *self)
{
	...
	if (self->spawnflags & TRAIN_ANIMATED)
		self->s.effects |= EF_ANIM_ALL;
	if (self->spawnflags & TRAIN_ANIMATED_FAST)
...
target: points to the func_train that is the elevator itself.

This is used in combination with a trigger that has a "pathtarget"
value of the path_corner you want the elevator to move to when the
trigger is used.

For example, if you have a func_button whose "target" is a trigger_elevator
whose "target" is a func_train, and you press the button, the func_train
will travel to the path_corner the trigger_button has as its "pathtarget".
*/
...
void SP_func_timer (edict_t *self)
{
	...
	if (self->spawnflags & SPAWNFLAG_FT_START_ON)
	{
...
{
	// If on, turn off
	if (self->spawnflags & SPAWNFLAG_FC_START_ON)
	{
		self->speed = 0;
		self->spawnflags &= ~SPAWNFLAG_FC_START_ON;
	}
	else	// turn on
	{
	...
	if (!(self->spawnflags & SPAWNFLAG_FC_TOGGLE))
		self->count = 0;
}
...
void door_secret_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	//  RSP 022000 enable secret door sounds
	if (self->moveinfo.sound_start)
		gi.sound (self, CHAN_NO_PHS_ADD+CHAN_VOICE, self->moveinfo.sound_start, 1, ATTN_STATIC, 0);
	self->s.sound = self->moveinfo.sound_middle;
